...
country = {
	...
	AI_SUPPORT_REFORM = 0.25, -- At least this many % needs to support a reform for the AI to take it
	...
	NAVAL_BASE_SUPPLY_SCORE_BASE = 6, -- base value that is powered by level of naval base. Determines the naval supplying capabilities.
	NAVAL_BASE_SUPPLY_SCORE_EMPTY = 1.2, -- min value for coastal provinces with no naval base.
	NAVAL_BASE_NON_CORE_SUPPLY_SCORE = 0.3, -- modifier for supply score for naval bases that are not in core provinces.
	COLONIAL_POINTS_FROM_SUPPLY_FACTOR = 0.5,
	COLONIAL_POINTS_FOR_NON_CORE_BASE = 0.5,
	...
	RESEARCH_POINTS_ON_CONQUER_MULT = 7500, -- multiplier to RP got by conquering as unciv when got enacted military reforms
...
economy = {
	...
	RGO_SUPPLY_DEMAND_FACTOR_FIRE = 0.8,		-- how fast pops are Fired when there is a low demand
	...
	TRADE_CAP_LOW_LIMIT_NAVAL = 0, 			-- the lowest % the slider can go for naval units
...
military = {
	...
	LEADER_AGE_DEATH_FACTOR = 80, -- higher value means leaders live longer
	LEADER_PRESTIGE_TO_MORALE_FACTOR = 2.8, -- f.ex. 100% of prestige = +10% morale
	LEADER_PRESTIGE_TO_MAX_ORG_FACTOR = 2.7, -- f.ex. 100% of prestige = +10 max org
	...
	AI_BIGSHIP_PROPORTION = 0.50, -- fraction of ships in the navy that should be ships of the line
	AI_LIGHTSHIP_PROPORTION = 0.40, -- fraction of ships in the navy that should be cruisers, frigates etc
	AI_TRANSPORT_PROPORTION = 0.10, -- fraction of ships in the navy that should be should be transports
	AI_CAVALRY_PROPORTION = 0.40, -- fraction of brigades that should be cavalry
	AI_SUPPORT_PROPORTION = 0.90, -- fraction of brigades that should be artillery and other support units
	AI_SPECIAL_PROPORTION = 0.60, -- fraction of brigades that should be engineers and tanks
	AI_ESCORT_RATIO = 5.0, -- ratio of escorts to transports in invasion fleets
	AI_ARMY_TAXBASE_FRACTION = 3.0, -- max of tax base that AI will spend on army supply costs (based on peacetime costs)
	AI_NAVY_TAXBASE_FRACTION = 3.0, -- max of tax base that AI will spend on navy supply costs (based on peacetime costs)
	AI_BLOCKADE_RANGE = 6000, -- max distance the AI will send out blockade fleets from their home base
	...
	SIEGE_BRIGADES_MAX = 25, -- the number of brigades above which you get no addition benefit in sieges
	SIEGE_BRIGADES_BONUS = 1.0, -- the bonus to siege speed from each brigade
	...
	BASE_MILITARY_TACTICS = 2.5, -- base mil tactics before tech
...
ai =
{
	COLONY_WEIGHT = 1000.0, -- ai weight for colonising
	ADMINISTRATOR_WEIGHT = 10.0, -- ai weight for new bureaucrat
	INDUSTRYWORKER_WEIGHT = 100000.0, -- ai weight for new industry workers
	EDUCATOR_WEIGHT = 100000.0, -- ai weigth for new clergy
	...
	CLERGYMEN_FRACTION = 1, -- max amount of population AI wants to be clergymen
	PRODUCTION_WEIGHT = 5, -- ai weight for new production
	...
	POP_PROJECT_INVESTMENT_MAX_BUDGET_FACTOR = 2, -- how much % of our current budget can be spend on the pop project investments.
	...
	CHANCE_BUILD_FORT = 1,
	CHANCE_INVEST_POP_PROJ = 200,
	CHANCE_FOREIGN_INVEST = 100,
	TWS_AWARENESS_SCORE_LOW_CAP = 0.05, -- AI will always add CBs if current warscore is less then that number (including TWS)
	TWS_AWARENESS_SCORE_ASPECT = 0.0, -- AI will not add any more CBs when TWS is more then X% of total WS. (to not destroy the progress)
	PEACE_BASE_RELUCTANCE = 50, -- AI base stubbornness to refuse peace (always applied)
	...
	AGGRESSION_BASE = 1000, -- general AI aggression
	AGGRESSION_UNCIV_BONUS = 300, -- additional AI civ aggression against uncivs
	...
	MIN_FLEETS = 5, -- Minimum amount of main fleets the AI will divide its navy into (does not include specialized fleets such as blockades and naval invasions)
	MAX_FLEETS = 30, -- Maximal amount of main fleets the AI will divide its navy into (does not include specialized fleets such as blockades and naval invasions)
